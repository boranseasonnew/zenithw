...
function progressFromLine(line) {
  const match = line.match(/\[download\]\s+([\d.]+)%.*?\s+at\s+(.+)/i);
  return match ? { percent: Number(match[1]), detail: match[2] } : null;
}
...
function runTool(file, args, timeout = 180000) {
  return new Promise((resolve, reject) => {
    const child = spawn(file, args, { windowsHide: true });
    let stdout = '', stderr = '';
    let settled = false;
    const done = (callback) => { if (settled) return; settled = true; clearTimeout(timer); callback(); };
    const timer = setTimeout(() => { child.kill(); done(() => reject(new Error('İşlem zaman aşımına uğradı.'))); }, timeout);
    child.stdout.on('data', (chunk) => { stdout += chunk; });
    child.stderr.on('data', (chunk) => { stderr += chunk; });
    child.on('error', (error) => done(() => reject(error)));
    child.on('close', (code) => done(() => code === 0 ? resolve({ stdout, stderr }) : reject(new Error((stderr || stdout).trim()))));
  });
}
...
handleMain('download:start', (_, request) => {
  ...
  const record = { child, job, key, workDir, destination: current.downloadFolder, terminal: false };
  activeJobs.set(job.id, record);
  emit(job, 'started', { title: request.title || job.url });
  let tail = '', errorTail = '';
  child.stdout.on('data', (chunk) => { tail += chunk.toString(); const lines = tail.split(/\r?\n/); tail = lines.pop(); for (const line of lines) { const progress = progressFromLine(line); if (progress) emit(job, 'progress', progress); } });
  child.stderr.on('data', (chunk) => { errorTail = `${errorTail}\n${chunk.toString()}`.slice(-6000); });
  child.on('error', (error) => { if (record.terminal) return; record.terminal = true; activeJobs.delete(job.id); fs.rmSync(workDir, { recursive: true, force: true }); emit(job, 'failed', { detail: error.message }); });
  child.on('close', (code) => {
